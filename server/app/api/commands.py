...
from app.auth import role_required, verify_agent_token  # ✅ добавлен verify_agent_token
...
# Блокируем опасные паттерны (defence-in-depth поверх whitelist)
BLOCKED_PATTERNS = [
    r"(rm\s+-rf\s*/)", r"(mkfs\.)", r"(>\s*/dev/sd)",
...
def issue_command():
    ...
    if not is_safe_command(command_text):
        log_action(get_jwt_identity(), "user", "blocked_dangerous_command",
    ...
    push_command(agent.id, cmd.id)  # уведомить агент через WS
    log_action(get_jwt_identity(), "user", "command_issued",
               target=agent.hostname, details={"cmd_id": cmd.id, "type": cmd_type})
...
def submit_result(cmd_id):
    ...
    from app.auth import agent_auth_required
    # inline check для простоты
...
def get_result(cmd_id):
    """Администратор читает результат выполненной команды."""
    cmd = db.session.get(Command, cmd_id)
    if not cmd:
...
def list_agent_commands(agent_id):
    """История команд для конкретного агента."""
    limit = min(request.args.get("limit", 20, type=int), 100)
    cmds  = Command.query.filter_by(agent_id=agent_id)\
...
def get_pending(agent_id):
    """Fallback polling — агент забирает команды если WS недоступен."""
    ...
    # Берём из in-process очереди
    from app.command_queue import pop_pending
    cmd_ids = pop_pending(agent_id)

    # Также берём из БД всё что pending (на случай рестарта сервера)
