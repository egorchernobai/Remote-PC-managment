...
from app.auth import role_required, agent_auth_required
...
def register_agent():
    """Агент регистрируется, получает уникальный токен."""
    ...
    # Возвращаем токен ТОЛЬКО один раз при регистрации
...
def list_agents():
    ...
    return jsonify([{
        ...
        "status": a.status, "last_seen": str(a.last_seen),
        "os_info": a.os_info
    } for a in agents])
...
def heartbeat(agent_id):
    """Агент отправляет системную информацию каждые N секунд."""
    data = request.get_json(force=True)
    agent = request.agent
