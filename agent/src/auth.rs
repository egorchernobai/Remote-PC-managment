...
use reqwest::Client;
use serde::Deserialize;  // убрали Serialize
use serde_json::json;
...
pub async fn ensure_registered(client: &Client, cfg: &Config) -> Result<AgentState> {
    ...
    if state.agent_id.is_some() && state.token.is_some() {
        if verify_registration(client, cfg, &state).await {
            info!("Agent already registered: {}", state.agent_id.as_deref().unwrap());
            return Ok(state);
        }
        warn!("Server doesn't recognize credentials, re-registering...");
        // Присваиваем напрямую без промежуточной переменной
...
pub fn build_http_client(cfg: &Config) -> Result<Client> {
    use std::fs;
    // убрали sync::Arc — не нужен здесь
    ...
    // Строим rustls ClientConfig для reqwest
    ...
    // ✅ from_pem — cert идёт ПЕРЕД key в одном буфере
    let mut identity_pem = cert_pem.clone();
    identity_pem.extend_from_slice(&key_pem);
    let identity = reqwest::Identity::from_pem(&identity_pem)?;

    Ok(Client::builder()
        .use_preconfigured_tls(tls_config)  // наш rustls config с кастомным CA
        .identity(identity)                 // клиентский сертификат для mTLS
