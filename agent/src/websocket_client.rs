...
use crate::{command_executor, config::Config, sysinfo_collector};
...
pub async fn run_forever_plain(
    ...
    loop {
        ...
        let result = match &connector {
            Some(c) => connect_tls_and_handle(&cfg, &agent_id, &token, c).await,
            None    => connect_plain_and_handle(&cfg, &agent_id, &token).await,
        };
        match result {
            Ok(_)  => { info!("Session ended"); delay_secs = 2; }
            Err(e) => {
...
async fn connect_tls_and_handle(
    cfg: &Config, agent_id: &str, token: &str, connector: &Connector,
) -> Result<()> {
    let (ws_stream, _) = connect_async_tls_with_config(
        &cfg.ws_url, None, false, Some(connector.clone()),
    ).await?;
    handle_ws(ws_stream, agent_id, token).await
}

async fn connect_plain_and_handle(cfg: &Config, agent_id: &str, token: &str) -> Result<()> {
    let (ws_stream, _) = connect_async(&cfg.ws_url).await?;
    handle_ws(ws_stream, agent_id, token).await
}

async fn handle_ws<S>(ws_stream: S, agent_id: &str, token: &str) -> Result<()>
where
     ...
     + Unpin,
{
    ...
    match read.next().await {
        Some(Ok(Message::Text(msg))) => {
            ...
            if v["type"] != "auth_ok" { anyhow::bail!("Auth rejected: {msg}"); }
    ...
    tokio::spawn(async move {
        loop {
            ...
            if sysinfo_tx.send(msg).is_err() { break; }
        }
    });
        ...
        tokio::select! {
            incoming = read.next() => match incoming {
                ...
                Some(Ok(Message::Text(text))) => {
                    if let Some(r) = handle_server_message(&text).await { tx.send(r)?; }
                }
                _ => {}
            },
...
async fn handle_server_message(text: &str) -> Option<String> {
    ...
    match data["type"].as_str() {
        Some("execute") => {
            ...
            let resp = match command_executor::execute(&cmd_id, &cmd_type, &command).await {
                Ok(r)  => json!({"type":"command_result","command_id":r.command_id,
                                 "output":r.output,"exit_code":r.exit_code}),
                Err(e) => { error!("Exec error: {e}");
        ...
        other => { warn!("Unknown type: {other:?}"); None }
    }
}
